...
File::File(const std::string &file_name)
{
    ...
    ownRead(&FileHeader.headers_num, sizeof(uint32_t), 1, file);
    ownRead(&FileHeader.file_header_size, sizeof(uint32_t), 1, file);
    ownRead(FileHeader.headers, sizeof(SegmentsHeader), Headers::HEADERS_NUM, file);

    // Read FileStringHeader
    ownRead(&FileStringHeader.num_of_strings, sizeof(uint16_t), 1, file);
    ownRead(&FileStringHeader.header_size, sizeof(uint32_t), 1, file);

    // Read FileFunctionHeader
    ownRead(&FileFuncHeader.num_of_functions, sizeof(uint32_t), 1, file);
    ownRead(&FileFuncHeader.header_size, sizeof(uint32_t), 1, file);
    ...
    // Read String Section
    std::vector<FileString> strings(FileStringHeader.num_of_strings);
    fseek(file, FileHeader.headers[LITERALS].offset_from_start, SEEK_SET);
    for (auto &string_info : strings) {
        ownRead(&string_info.id, sizeof(uint16_t), 1, file);
        ownRead(&string_info.str_size, sizeof(uint64_t), 1, file);
    ...
    // Read Func Section
    std::vector<FileFunction> functions(FileFuncHeader.num_of_functions);
    fseek(file, FileHeader.headers[FUNCTIONS].offset_from_start, SEEK_SET);
    for (auto &func_info : functions) {
        ownRead(&func_info.id, sizeof(FuncId), 1, file);
        ownRead(&func_info.func_start, sizeof(ByteOffset), 1, file);
        ownRead(&func_info.name_size, sizeof(uint64_t), 1, file);
        ownRead(&func_info.num_of_args, sizeof(uint8_t), 1, file);
        ownRead(&func_info.num_of_vregs, sizeof(uint16_t), 1, file);
...
void File::fillCodeHeader()
{
    ...
    codeHeader.offset_from_start =
        FileHeader.file_header_size + FileStringHeader.header_size + FileFuncHeader.header_size;
...
void File::fillStringsHeader()
{
    ...
    FileStringHeader.num_of_strings = Strings.size();
    stringHeader.size = 0;
...
void File::fillFunctionsHeader()
{
    ...
    funcHeader.offset_from_start = FileHeader.file_header_size + FileFuncHeader.header_size +
                                   ...
                                   FileStringHeader.header_size;
    FileFuncHeader.num_of_functions = Functions.size();
    funcHeader.size = 0;
...
void File::serializeFileHeader(std::FILE *out)
{
    ...
    ownWrite(&FileHeader.headers_num, sizeof(uint32_t), 1, out);
    ownWrite(&FileHeader.file_header_size, sizeof(uint32_t), 1, out);
    ownWrite(FileHeader.headers, sizeof(SegmentsHeader), Headers::HEADERS_NUM, out);
}

void File::serializeHeaders(std::FILE *out)
{
    ownWrite(&FileStringHeader.num_of_strings, sizeof(uint16_t), 1, out);
    ownWrite(&FileStringHeader.header_size, sizeof(uint32_t), 1, out);
    ownWrite(&FileFuncHeader.num_of_functions, sizeof(uint32_t), 1, out);
    ownWrite(&FileFuncHeader.header_size, sizeof(uint32_t), 1, out);
}
...
void File::dumpFileHeader()
{
    ...
    std::cout << "Binary File Name: " << FileHeader.bin_file_name << std::endl;
    std::cout << "Num of Headers: " << FileHeader.headers_num << std::endl;
    std::cout << "File Header Size: " << FileHeader.file_header_size << std::endl;
}

void File::dumpCodeHeader()
{
    auto &codeHeader = FileHeader.headers[CODE];
    std::cout << "Code Size: " << codeHeader.size << std::endl;
    std::cout << "Code Header Offset To Segment From Start: " << codeHeader.offset_from_start << std::endl;
}
...
void File::dumpStringHeader()
{
    auto &stringPreHeader = FileHeader.headers[LITERALS];
    std::cout << "String Pre Header Size: " << stringPreHeader.size << std::endl;
    std::cout << "String Pre Header Offset To Segment From Start: " << stringPreHeader.offset_from_start << std::endl;
    std::cout << "Num of strings: " << FileStringHeader.num_of_strings << std::endl;
    std::cout << "String Header Size: " << FileStringHeader.header_size << std::endl;
}

void File::dumpString()
{
    for (auto &str : Strings) {
        std::cout << "String Id: " << static_cast<uint32_t>(str.id) << std::endl;
        std::cout << "String size: " << str.str_size << std::endl;
        std::cout << "String: " << str.str << std::endl;
    }
}

void File::dumpFunctionHeader()
{
    auto &funcPreHeader = FileHeader.headers[FUNCTIONS];
    std::cout << "Function Pre Header Size: " << funcPreHeader.size << std::endl;
    std::cout << "Function Pre Header Offset To Segment From Start: " << funcPreHeader.offset_from_start << std::endl;
    std::cout << "Num of functions: " << FileFuncHeader.num_of_functions << std::endl;
    std::cout << "Function Header Size: " << FileFuncHeader.header_size << std::endl;
}

void File::dumpFunction()
{
    for (auto &func : Functions) {
        std::cout << "Func Id: " << static_cast<uint32_t>(func.id) << std::endl;
        std::cout << "Func offset: " << func.func_start << std::endl;
        std::cout << "Func name_size: " << func.name_size << std::endl;
        std::cout << "Func num_of_args: " << static_cast<uint32_t>(func.num_of_args) << std::endl;
        std::cout << "Func num_of_vregs: " << func.num_of_vregs << std::endl;
